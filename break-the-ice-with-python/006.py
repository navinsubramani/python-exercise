# Write a program that calculates and prints the value according to the given formula:
# Q = Square root of[(2 * C * D)/H]
# Following are the fixed values of C and H:
# C is 50. H is 30.
# D is the variable whose values should be input to your program in a comma-separated sequence.For example Let us assume the following comma separated input sequence is given to the program: 100, 150, 180
# The output of the program should be: 18, 22, 24

from math import sqrt
from functools import reduce


def calculate_Q(D):
    C = 50
    H = 30
    return round(sqrt((2*C*D)/H))


li_D = input('Enter the D in a sequence with comma seperated value:').split(',')
li_D = [int(x) for x in li_D]
# li_Q = [calculate_Q(x) for x in li_D]
li_Q = list(map(calculate_Q, li_D))
print(li_Q)
